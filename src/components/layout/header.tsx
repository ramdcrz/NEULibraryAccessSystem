...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-[100] w-full bg-background/5 backdrop-blur-3xl border-b border-black/5 dark:border-white/10 flex h-16 sm:h-20 items-center gap-4 sm:gap-8 px-4 sm:px-6 md:px-12 transition-all">
      <div className="flex items-center gap-4 sm:gap-8 py-2 flex-shrink-0">
        <Link href="/" className="flex items-center gap-2.5 group transition-opacity hover:opacity-80 py-1 w-max">
          <div className="w-8 h-8 sm:w-10 sm:h-10 shrink-0 rounded-xl blue-gradient text-white shadow-md flex items-center justify-center aspect-square transition-transform group-active:scale-95">
            <BookMarked className="h-4 w-4 sm:h-5 sm:w-5" />
          </div>
          <div className="flex flex-col justify-center w-max">
            <span className="text-base sm:text-lg lg:text-xl font-extrabold leading-none text-blue-gradient w-max">
              NEU Library
            </span>
            <span className="text-[7px] sm:text-[8px] lg:text-[9px] font-bold uppercase tracking-[0.28em] sm:tracking-[0.32em] lg:tracking-[0.38em] text-muted-foreground mt-0.5 w-full text-center block">
              Access System
            </span>
          </div>
        </Link>
